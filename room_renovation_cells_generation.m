...
%% renovation_cells_paths should be generated from renovation_cells_waypaths 
%% input:renovation_cells_waypaths
%% output:renovation_cells_paths
for i=1:1:size(renovation_cells_waypaths,2)
    for j=1:1:size(renovation_cells_waypaths{i},2)
        for k=1:1:size(renovation_cells_waypaths{i}{j},2)
            ...
            b=path_clustering(a);
            renovation_cells_paths{i}{j}{k}=b;
        end
    end
end
...
for i=1:1:size(renovation_cells_paths,2)
    for j=1:1:size(renovation_cells_paths{i},2)
        for k=1:1:size(renovation_cells_paths{i}{j},2)
            paths_points=[];
            for m=1:1:size(renovation_cells_paths{i}{j}{k},1)
                paths_points(2*m-1,1:3)=renovation_cells_paths{i}{j}{k}(m,1:3)*0.001;
                paths_points(2*m-1,4:6)=[theta_x,theta_y(i),theta_z];
                paths_points(2*m,1:3)=renovation_cells_paths{i}{j}{k}(m,4:6)*0.001;
                paths_points(2*m,4:6)=[theta_x,theta_y(i),theta_z];
            end
            % path_points=unique(path_points,'rows');
            renovation_cells_waypioints_onpath{i}{j}{k}=paths_points;
        end
    end
end
...
for i=1:1:size(renovation_cells_paths,2)
    for j=1:1:size(renovation_cells_paths{i},2)
        ...
        for k=1:1:size(renovation_cells_paths{i}{j},2)
            for m=1:1:size(renovation_cells_paths{i}{j}{k},1)
                xy_vector(n,1)=renovation_cells_paths{i}{j}{k}(m,1);
                xy_vector(n,2)=renovation_cells_paths{i}{j}{k}(m,2);
                n=n+1;
            end
        end
...
for i=1:1:size(renovation_cells_paths,2)
    for j=1:1:size(renovation_cells_paths{i},2)
        for k=1:1:size(renovation_cells_paths{i}{j},2)
            num=size(renovation_cells_paths{i}{j}{k},1);
            if mod(num,2)==1
                z1=renovation_cells_paths{i}{j}{k}(1,3);
                z2=renovation_cells_paths{i}{j}{k}(end,3);
                z_mean=(z1+z2)/2;
            else
                z1=renovation_cells_paths{i}{j}{k}(1,3);
                z2=renovation_cells_paths{i}{j}{k}(end,3);
...
%         for k=1:1:1
            ...
            for m=1:1:size(renovation_cells_waypioints_onpath{i}{j}{k},1)
            % for m=1:1:1
                renovationpathwaypoints_xyz=renovation_cells_waypioints_onpath{i}{j}{k}(m,1:3);
                renovationpathwaypoints_rpy=renovation_cells_waypioints_onpath{i}{j}{k}(m,4:6);
...
% the selection and reschedule of the above data
% input: renovation_cells_waypioints_onpath{}{}{}(,1:6)
    ...
    for j=1:1:size(renovation_cells_manipulatorbase_positions{i},2)
        ...
        for k=1:1:size(renovation_cells_manipulatorbase_positions{i}{j},2)
            ...
            if z-1.2>=0.01
                ...
                renovation_cells_waypioints_onpath1{i}{j}{n}=renovation_cells_waypioints_onpath{i}{j}{k};
                n=n+1;
            end
        end
    end
...
%% input: renovation_planes_edge_cell, room_plane_edge_cell
for i=1:1:size(renovation_planes_edge_cell,2)
    for j=1:1:size(renovation_planes_edge_cell{i},1)
        renovation_planes_edge_cell{i}(j,1:6)=renovation_planes_edge_cell{i}(j,1:6)*0.001;
    end
end
for i=1:1:size(room_plane_edge_cell,2)
    for j=1:1:size(room_plane_edge_cell{i},1)
        room_plane_edge_cell{i}(j,1:6)=room_plane_edge_cell{i}(j,1:6)*0.001;
    end
end


%% transform of all data, no rpy
...
%             for m3=1:1:size(manipulator_endeffector_positions_onpath2{i}{j}{k},1)
%                 point3=manipulator_endeffector_positions_onpath2{i}{j}{k}(m3,1:3);
%                 tran=rotz(pi/2)*point3';
%                 tran=tran';
%                 manipulator_endeffector_positions_onpath2{i}{j}{k}(m3,1:3)=tran+tran1;
%                 
%                 rpy=manipulator_endeffector_positions_onpath2{i}{j}{k}(m3,4:6);
%                 manipulator_endeffector_positions_onpath2{i}{j}{k}(m3,4:6)=rpy;
%             end
        end
    end
end
...
for i=1:1:size(renovation_planes_edge_cell,2)
    for j=1:1:size(renovation_planes_edge_cell{i},1)
        point1=renovation_planes_edge_cell{i}(j,1:3);
        point2=renovation_planes_edge_cell{i}(j,4:6);
...
for i=1:1:size(room_plane_edge_cell,2)
    for j=1:1:size(room_plane_edge_cell{i},1)
        point1=room_plane_edge_cell{i}(j,1:3);
        point2=room_plane_edge_cell{i}(j,4:6);
...
%% some manipulator base positions are not suitable for the field experiment
%% therefore here we have some deleting process for it
